...
bool check(char* word)
{
    ...
    {
        index = (tolower(word[i])) - 'a';  //assign an index for each letter of the word inputed, allowing to search for the word in children[]

        if(cursor->children[index] == NULL) //the letter wasnt found, meaning that word isnt possible in this sequence
        {
            return false;
        }
        cursor = cursor->children[index];  //if the letter exists, reassign pointer to that node, and resets process for other letters
    }
